...
function applyToShellCode(code: string, mode: BashPolicyMode, indexPath?: string): ToolPolicyAction {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const result = applyBashPolicy(line, hasHeredoc ? "block" : mode, { indexPath });
...
function applyJsChildProcessGuard(code: string, mode: BashPolicyMode): ToolPolicyAction {
  ...
  for (let k = calls.length - 1; k >= 0; k--) {
    ...
    const result = applyBashPolicy(parse.content, mode);
    if (result.action === "block") {
      return {
        action: "block",
...
export function applyToolCallPolicy(
  toolName: string,
  ...
  if (key === "ctx_batch_execute") {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      const result = applyBashPolicy(rec.command, mode, context);
      if (result.action === "block") {
  ...
  if (key === "ctx_execute" || key === "ctx_execute_file") {
    ...
    if (language !== "shell") {
      ...
      const guard = applyJsChildProcessGuard(code, mode);
    ...
    const result = applyToShellCode(code, mode, context?.indexPath);
